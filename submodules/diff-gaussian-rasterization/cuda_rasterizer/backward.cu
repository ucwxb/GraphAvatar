	...
	float* dL_dcov)
{
    ...
    ///--------------------compute grad ---------------------
    float3 dL_dconic = { dL_dconics[4 * idx], dL_dconics[4 * idx + 1], dL_dconics[4 * idx + 3] };  //xyyz
	float denom = a * c - b * b;
    ...
    dL_dtz += dL_ddepths[idx];  //Partial derivatives from depth
...
renderCUDA(
    ...
    const float* __restrict__ final_Ws,
...
//    float Dep = D_final;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
            ...
            const float dD_ddepth = alpha *T/Wei;
            ...
            {   //+dW_dalpha
                const float dD_dW = -Dep/Wei;
                accum_wei = last_alpha + (1.f - last_alpha) * accum_wei;
                dL_dalpha += (1.f - accum_wei) * dL_dD * dD_dW;
            }
            {   //+dsum_dalpha
                const float dD_dsum = 1.f/Wei;
                const float d = collected_depths[j];
                accum_dep = last_alpha * last_depth + (1.f - last_alpha) * accum_dep;
                last_depth = d;
                dL_dalpha += (d - accum_dep) * dL_dD * dD_dsum;
            }
			dL_dalpha *= T;
			...
			// print len of bg_color and dL_dpixel
...
void BACKWARD::preprocess(
	...
	const float tan_fovx, float tan_fovy,
	const glm::vec3* campos,
	const float3* dL_dmean2D,   //[in]
	const float* dL_dconic,     //[in]
	...
	glm::vec4* dL_drot)         //[output]
{
	...
	computeCov2DCUDA << <(P + 255) / 256, 256 >> > (
		...
		viewmatrix,
		dL_dconic,
        dL_ddepths,             //[in, from BackWARD::render]
		(float3*)dL_dmean3D,    //[output], COV3D to cov2d to covnic
		dL_dcov3D);             //[output]
...
void BACKWARD::render(
	...
	const float* colors,            //color of each GS
    const float* depths,            //depth of each GS
	const float* final_Ts,          //imgState.accum_alpha
    const float* final_Ws,          //imgState.accum_weight
